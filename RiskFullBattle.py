...
print
print
print "Welcome to the Risk Simulator"
print
while True:
	legit_number = 1
...
print
while True:
	defender_retreat_query = raw_input("Does the Defender want a retreat condition? ").lower()
	if defender_retreat_query == "yes" or defender_retreat_query == "y":
		break
	if defender_retreat_query == "no" or defender_retreat_query == "n":
		break
	else:
		print
		print "Please enter a valid response."
...
if defender_retreat_query == "yes" or defender_retreat_query == "y":
	while True:
		legit_number = 1
		defender_retreat_response = raw_input("Enter standing armies at time of retreat: ")
		try:
			i = float(defender_retreat_response)
		except ValueError, TypeError:
			print
			print "Please enter a valid number."
			legit_number = 0
		if legit_number == 1:
			if int(defender_retreat_response) >= defender_number:
				print
				print "Please enter a number less than your army size."
			elif int(defender_retreat_response) < 1:
				print
				print "Please enter a valid response."
			else:
				defender_retreat = int(defender_retreat_response)
				break
		else:
			pass
		print
...
while True:
	...
	if defender_number == defender_retreat:
		attacker_number_final = str(attacker_number)
		defender_number_final = str(defender_retreat)
		print "Defender retreats %s soldier(s). Attacker advances with %s soldiers." % (defender_number_final, attacker_number_final)
		break
	if attacker_number == 1 or defender_number == 0:
		attacker_number_final = str(attacker_number)
		defender_number_final = str(defender_number)
		if attacker_number == 1:
			print "Attacker fails. Defender has %s soldier(s) standing." % (defender_number_final)
			print
			import subprocess
			audio_file = "/Users/elwynpratt/Documents/warhorn.mp3"
			return_code = subprocess.call(["afplay", audio_file])
		if defender_number == 0:
			print "Attacker is victorious with %s soldier(s) standing." % (attacker_number_final)
			print
			import subprocess
